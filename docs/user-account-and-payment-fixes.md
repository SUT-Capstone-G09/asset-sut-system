...
- เพิ่ม watermark ลายจุดบางๆ ที่พื้นหลัง
...
- เปลี่ยนหัวข้อจาก centered hero เป็น **left-aligned พร้อม eyebrow label** เล็กๆ ด้านบน
- แบ่งลำดับความสำคัญจริง: **ห้องประชุม** (หมวดที่จองบ่อยสุด) ได้การ์ดใหญ่แบบ hero แยกเดี่ยว มี tag "ยอดนิยม" ส่วนอีก 3 หมวดเป็น **list แบบมีเลขกำกับ (01/02/03)** คั่นด้วยเส้นบาง แทนกล่องการ์ดซ้ำๆ
...
**iteration 3a:** กริด 2×2 เท่ากันทุกใบ + ghost icon ขนาดใหญ่จางๆ ที่มุมการ์ดแทน badge + เส้น accent สีบางที่ก้นการ์ด
...
หน้าเดิมเป็นแค่ placeholder ("Profile Page") เปล่าๆ สร้างใหม่ทั้งหมด:
...
- บัญชีที่สมัครก่อนวันที่ 22 มิ.ย. 69 (เช่น user 4, 5, 6 ที่เจอตอนตรวจสอบ) ข้อมูลตกค้างอยู่ในตาราง `requesters` เก่า ไม่มีแถวใน `profiles` เลย → ทุกจุดที่พึ่ง `requesterRepo.FindByUserID()` (คำนวณราคา booking, `GET /me/requester`) หาไม่เจอและ fallback ผิดแบบเงียบๆ
...
4. **GORM save-association bug** — `FindByID` preload ความสัมพันธ์ `Status` ไว้ พอแก้ `StatusID` ตรงๆ แล้ว `Save()` ธรรมดาจะเขียนทับกลับเป็นค่าเดิมจาก association ที่ preload ไว้ (ทั้งใน payment และ invoice repo) → แก้เป็น `.Omit(clause.Associations)` ก่อน save
...
**ปัญหา:** เงื่อนไข `done = idx < stepIdx` ทำให้ขั้นตอนสุดท้ายไม่มีทางเป็น `done` ได้เลย (ไม่มีขั้นที่ 5 ให้เทียบว่าน้อยกว่า) แม้ booking จะสถานะ `completed` จริงแล้ว ขั้นสุดท้ายก็ยังโชว์แค่วงกลมเลข 4 เฉยๆ
