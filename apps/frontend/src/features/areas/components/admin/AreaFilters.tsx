  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between bg-card p-4 rounded-2xl border border-border/50 shadow-sm">
      ...
      <div className="flex flex-wrap sm:flex-nowrap items-center gap-2">
        {/* View Toggle */}
        <div className="flex items-center bg-slate-100/80 backdrop-blur-sm rounded-lg p-1 gap-1 border border-slate-200/50 shadow-inner h-10 shrink-0">
          <button
            type="button"
        ...
        {hasActiveFilters && (
          <button
            ...
            onClick={onReset}
            title="ล้างตัวกรอง"
            className="size-10 flex items-center justify-center text-muted-foreground bg-background border border-border/80 rounded-xl hover:bg-slate-50 hover:text-foreground transition-colors shrink-0"
          >
            <RotateCcw size={15} />
          </button>
        )}
