...
export default function AdminTenantDetailView({
  ...
  const [notes, setNotes] = useState<string>(initialTenant.note || "");
  ...
  const [address, setAddress] = useState("123/45 สุขุมวิท ซอย 21, คลองเตยเหนือ, วัฒนา, กรุงเทพมหานคร 10110, ประเทศไทย");
  ...
  const currentContract = tenant.contracts?.find(c => c.status === "active" || c.status === "expiring") || {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-16 relative">
      ...
      {/* Main Reorganized Layout: Left Column (Profile & Contract Spec) - 5 Cols, Right Column (History Timeline) - 7 Cols */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
                ...
                {isEditingProfile ? (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-xs font-bold px-3 py-1.5 rounded-lg bg-brand-primary text-white hover:bg-brand-primary/90 transition-all flex items-center gap-1 shadow-sm"
                    >
                      <Save size={12} />
                      บันทึก
                    </button>
                  ...
                  <button
                    ...
                    className="text-xs font-bold px-3 py-1.5 rounded-lg bg-slate-50 text-slate-655 hover:bg-slate-100 hover:text-slate-900 border border-slate-200/60 transition-all flex items-center gap-1"
                  >
                    <Edit2 size={12} />
                    แก้ไขข้อมูล
                  </button>
                ...
                <div className="space-y-4 mb-6">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all font-semibold text-slate-700"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all font-semibold text-slate-700 min-h-[70px] resize-none"
                    />
                  </div>
                </div>
              ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-slate-700 font-semibold text-sm flex items-center gap-2">
                        ...
                        {tenant.nationalId || "1-3012-00445-67-8"}
                      </span>
                    </div>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full text-slate-700 font-semibold text-sm border border-slate-200 rounded-xl p-3 pb-12 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-800 transition-all min-h-[80px] resize-none"
                />
                <button
                  type="button"
                  onClick={() => showToast("บันทึกโน้ตสำเร็จ", "success")}
                  className="absolute right-2.5 bottom-2.5 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg transition-colors shadow-xs"
                >
                  บันทึก
                </button>
              </div>
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-3">
            ...
            <div className="relative aspect-video w-full rounded-2xl overflow-hidden border border-slate-200 bg-slate-950/90 group shadow-inner">
              <Image
              ...
              <div className="absolute bottom-3 left-3 right-3 bg-slate-900/90 backdrop-blur-md px-3 py-2 rounded-xl border border-white/10 flex items-center justify-between">
                <span className="text-[10px] text-white font-medium truncate max-w-[70%]">บัตรประชาชน_{tenant.ownerName}.png</span>
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="pt-2 border-t border-slate-100 space-y-2">
              ...
              <p className="text-slate-700 text-xs font-semibold leading-relaxed bg-slate-50 p-3.5 rounded-xl border border-slate-100">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm flex flex-col h-full min-h-[550px]">
            <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
              <div className="flex items-center gap-2">
            ...
            <div className="flex-1 space-y-5">
              {tenant.contracts && tenant.contracts.length > 0 ? (
                tenant.contracts.map((contract) => {
                  ...
                  return (
                    <div 
                      key={contract.id} 
                      className={cn(
                        "p-5 rounded-2xl border transition-all relative overflow-hidden flex flex-col md:flex-row justify-between md:items-center gap-4",
                        isActiveOrExpiring
                          ? "bg-slate-50/70 border-brand-primary/20 hover:border-brand-primary/40 shadow-xs"
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-4 text-xs font-semibold text-slate-600">
                          <span>ค่าบำรุง: {contract.monthlyRental.toLocaleString()} บ./ด.</span>
                      ...
                      <button
                        ...
                        className="bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-black flex items-center justify-center gap-1.5 transition-all self-start md:self-auto shrink-0 shadow-xs"
                      >
                        <FileText size={14} />
                        ดูเอกสารสัญญา
                      </button>
                ...
                <div className="p-5 rounded-2xl border border-brand-primary/20 bg-slate-50/70 flex flex-col md:flex-row justify-between md:items-center gap-4 relative">
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500" />
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-slate-700 uppercase">CT-2024-0089</span>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
                      <Calendar size={13} />
                      <span>{formatThaiDate(tenant.contractStartDate || "2024-01-01")} - {formatThaiDate(tenant.contractEndDate)}</span>
                    </div>
                    <div className="flex items-center gap-4 text-xs font-semibold text-slate-600">
                      <span>ค่าบำรุง: 5,000 บ./ด.</span>
                      <span>ประกัน: {(tenant.deposit || 10000).toLocaleString()} บ.</span>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className="bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-black flex items-center justify-center gap-1.5 transition-all self-start md:self-auto shrink-0"
                  >
                    <FileText size={14} />
                    ดูเอกสารสัญญา
                  </button>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100 bg-slate-50 p-6 rounded-2xl border border-slate-200/60">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  type="button"
                  onClick={() => router.push(`/admin/contracts/actions?tenantId=${tenant.id}&action=renew`)}
                  className="flex-1 bg-brand-primary hover:bg-brand-primary/95 text-white font-bold py-3.5 px-4 rounded-xl text-xs flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md shadow-brand-primary/10"
                >
                  <RefreshCw size={14} />
                ...
                <button
                  type="button"
                  onClick={() => router.push(`/admin/contracts/actions?tenantId=${tenant.id}&action=terminate`)}
                  className="flex-1 bg-white hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-200 font-bold py-3.5 px-4 rounded-xl text-xs flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                >
                  <Ban size={14} />
      ...
      {selectedPreviewContract && isPreviewModalOpen && (
        <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] w-full max-w-3xl shadow-2xl flex flex-col max-h-[85vh] z-120 animate-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="bg-slate-200 px-6 py-4 flex items-center justify-between border-b border-slate-300 shrink-0">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => setIsPreviewModalOpen(false)}
                  className="ml-3 p-1 bg-slate-300/40 hover:bg-slate-300 rounded text-slate-600 transition-colors"
                >
                  <X size={16} />
                </button>
              </div>
            ...
            <div className="flex-1 p-8 overflow-y-auto flex justify-center bg-slate-100/90 items-start">
              <div
                ...
                className="bg-white w-[595px] min-h-[800px] shadow-lg border border-slate-200 p-12 transition-transform duration-200 relative flex flex-col justify-between select-none"
              >
                
                {/* Watermark */}
                <div className="absolute inset-0 flex items-center justify-center opacity-3 pointer-events-none z-0">
                  <div className="w-64 h-64 border-12 border-slate-900 rounded-full flex items-center justify-center font-black text-5xl text-slate-900 tracking-wider -rotate-45">
                    SUT
                  </div>
                </div>
                ...
                <div className="relative z-10 space-y-6 flex-1">
                  ...
                  <div className="space-y-3 pt-2 text-[9px] leading-relaxed text-slate-500">
                    ...
                    <div className="space-y-1.5 pt-1">
                      ...
                      <p>ผู้ให้เช่าตกลงให้เช่า และผู้เช่าตกลงเช่าพื้นที่ในบริเวณ {areaName} โซน {tenant.subLocation} เพื่อวัตถุประสงค์ในการทำธุรกิจประเภท {tenant.businessType}</p>
                    </div>
                ...
                <div className="relative z-10 grid grid-cols-2 gap-8 pt-8 border-t border-slate-100 mt-8">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-[9px] text-slate-500">({tenant.ownerName})</p>
                  </div>
                </div>
