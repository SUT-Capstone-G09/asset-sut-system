          ...
          <TableBody>
            {filteredData.map((item) => (
              <TableRow key={item.id} className="hover:bg-orange-50/30">
                ...
                <TableCell>
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${item.status === "ผ่าน" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                  >
                    {item.status}
                  </span>
                </TableCell>
                <TableCell
                  className={`text-center font-bold ${item.warningCount > 0 ? "text-red-600" : "text-slate-400"}`}
                >
                  {item.warningCount}
                </TableCell>
