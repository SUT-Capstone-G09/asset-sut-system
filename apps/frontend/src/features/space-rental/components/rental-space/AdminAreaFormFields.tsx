...
export default function AdminAreaFormFields({ isEdit = false, isLockedContext = false }: AdminAreaFormFieldsProps) {
  const { 
    register, 
    control,
    watch,
                ...
                render={({ field }) => (
                  <Select onValueChange={(val) => handleAreaChange(val, field.onChange)} value={field.value} disabled={isLockedContext}>
                    <SelectTrigger className={cn(
                      ...
                      isLockedContext && "opacity-75 bg-slate-100 cursor-not-allowed"
                    )}>
